...
def export_folder_contents_task(context, folder_path: str, output_dir: str):
    import pandas as pd
    ...
    output_name = generate_output_filename("FolderExport", folder_name, ".xlsx")
...
class FolderExporterPlugin(QtPlugin):
    plugin_id = "exporter"
    name = "Folder Exporter"
    description = "Scan a folder tree and export file metadata, paths, permissions, and dates into an Excel workbook."
    category = "Validation & Analysis"

    def create_widget(self, services) -> QWidget:
        return FolderExporterPage(services, self.plugin_id)


class FolderExporterPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Folder Exporter")
        title.setStyleSheet("font-size: 26px; font-weight: 700; color: #10232c;")
        layout.addWidget(title)

        description = QLabel(
            "Export file metadata for an entire folder tree into Excel, with a preview of the first rows inside the app."
        )
        ...
        self.folder_input.setPlaceholderText("Select a folder to export...")
        ...
        self.run_button = QPushButton("Export Folder")
        ...
        self.summary_label = QLabel("Choose a folder to export.")
    ...
    def _run(self) -> None:
        ...
        if not folder_path:
            QMessageBox.warning(self, "Missing Input", "Choose a folder to export.")
            return

        self.run_button.setEnabled(False)
        self.open_output_button.setEnabled(False)
        self.progress.setValue(0)
        self.summary_label.setText("Exporting folder contents...")
        self.table.setModel(None)
        self._table_model = None

        output_dir = str(self.services.default_output_path())
        self.services.run_task(
            lambda context: export_folder_contents_task(context, folder_path, output_dir),
    ...
    def _handle_result(self, payload: object) -> None:
        ...
        self.summary_label.setText(
            f"Exported {result['row_count']} rows for {result['folder_name']}. Previewing the first {len(result['dataframe'])} rows."
        )
        self.open_output_button.setEnabled(True)
        self.services.record_run(
            self.plugin_id,
            "SUCCESS",
            f"Exported folder metadata for {result['folder_name']}",
        )

    def _handle_error(self, payload: object) -> None:
        message = payload.get("message", "Unknown exporter error") if isinstance(payload, dict) else str(payload)
        self.summary_label.setText(message)
        self.services.record_run(self.plugin_id, "ERROR", message[:500])
        self.services.log("Folder exporter failed.", "ERROR")
